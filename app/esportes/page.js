    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-10">
          ...
          <p className="text-[#9ca3af]">Seu universo esportivo — notícias, análises e debates sobre Futebol, NBA e Fórmula 1.</p>
          <a
                      href="https://www.instagram.com/3wesports_?stkn=cDJrM3NmaWNvb3Ux"
            target="_blank"
      ...
      <NetshoesePromo />
          ...
          {/* Posts reais do Instagram @3wesports_ via Elfsight */}
      <ElfsightFeed
                    appId={ELFSIGHT_IDS.esportes}
        title="Últimos Posts — @3wesports_"
                  perfil="@3wesports_"
                            href="https://www.instagram.com/3wesports_?stkn=cDJrM3NmaWNvb3Ux"
                                    />
    </>
      ...
      <Link href={`/noticias/${noticia.slug}`} className="block">
        ...
        <div className="p-4">
          <div className="flex items-center gap-2 mb-2">
            <span className={`text-xs font-semibold ${CATEGORIA_COLORS[noticia.categoria] || 'text-[#9ca3af]'}`}>{noticia.categoria}</span>
          ...
          <p className="text-[#9ca3af] text-xs mt-2 line-clamp-2">{noticia.descricao}</p>
          <p className="text-[#9ca3af] text-xs mt-2">{noticia.tempo_leitura} min de leitura</p>
        </div>
      </Link>
    </article>
  );
