...
import { COMICS, LIVROS_RECOMENDADOS, NOTICIAS } from '@/lib/mock-data';
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-10">
          ...
          <p className="text-[#737373]">Quadrinhos, Animes, Filmes & Séries — notícias, curiosidades e bastidores do universo geek.</p>
          <a
        ...
        <div className="flex gap-2 flex-wrap mb-8">
          {['Todos', 'Marvel', 'DC Comics', 'HQ Indie', 'Livros'].map(tab => (
            <span key={tab} className={`px-4 py-1.5 rounded-full text-sm font-medium cursor-pointer transition-colors
              ${tab === 'Todos' ? 'bg-yellow-600 text-white' : 'bg-[#1a1a1a] text-[#737373] hover:text-white border border-[#2a2a2a]'}`}>
              {tab}
            </span>
          ))}
        </div>
        ...
        {/* Grid de HQs */}
        <section aria-labelledby="hqs-title" className="mb-12">
          <h2 id="hqs-title" className="text-xl font-bold text-white mb-5">HQs em Destaque</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {COMICS.map(comic => (
              <article key={comic.id} className="card-hover">
                <Link href={`/comics/${comic.slug}`} className="block">
                  <div className="relative aspect-[2/3] rounded-lg overflow-hidden bg-[#1a1a1a] mb-2">
                    <img src={comic.imagem} alt={`Capa: ${comic.titulo}`} loading="lazy" decoding="async" className="w-full h-full object-cover" />
                    <div className="absolute top-2 left-2">
                      <span className="text-xs bg-yellow-600/90 text-white px-1.5 py-0.5 rounded font-medium">{comic.categoria}</span>
                    </div>
                  </div>
                  <h3 className="text-white text-xs font-semibold leading-snug line-clamp-2 mb-1">{comic.titulo}</h3>
                  <p className="text-[#737373] text-xs">{comic.editora} · {comic.ano}</p>
                </Link>
              </article>
            ))}
          </div>
        ...
        {/* Livros recomendados */}
        <section aria-labelledby="livros-title">
          <h2 id="livros-title" className="text-xl font-bold text-white mb-5">📖 Livros Recomendados</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {LIVROS_RECOMENDADOS.map(livro => (
              <article key={livro.id} className="card-hover">
                <Link href={`/comics/${livro.slug}`} className="block">
                  <div className="aspect-[2/3] rounded-lg overflow-hidden bg-[#1a1a1a] mb-2">
                    <img src={livro.imagem} alt={livro.titulo} loading="lazy" className="w-full h-full object-cover" />
                  </div>
                  <h3 className="text-white text-xs font-semibold line-clamp-2 mb-1">{livro.titulo}</h3>
                  <p className="text-[#737373] text-xs">{livro.autor}</p>
                </Link>
              </article>
            ))}
          </div>
