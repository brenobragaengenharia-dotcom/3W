  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <p className="text-[#9ca3af] mb-8">As melhores séries para maratonar em todas as plataformas.</p>
            ...
            <article key={serie.id} className="card-hover">
              <Link href={`/series/${serie.slug}`} className="block">
                <div className="relative aspect-[2/3] rounded-lg overflow-hidden bg-[#1a1a1a] mb-2">
                  <Image src={serie.imagem} alt={`Capa: ${serie.titulo}`} fill sizes="(max-width: 640px) 50vw, (max-width: 768px) 33vw, (max-width: 1024px) 25vw, 16vw" className="object-cover" />
                  {serie.nota && <div className="absolute top-2 right-2 z-10"><span className="rating-badge">★ {serie.nota}</span></div>}
                </div>
        ...
        {NOTICIAS_SERIES.length > 0 && (
          <section aria-labelledby="noticias-series-title" className="mt-12">
            <div className="flex items-center justify-between mb-5">
              <h2 id="noticias-series-title" className="text-xl font-black text-white">Notícias de Séries</h2>
